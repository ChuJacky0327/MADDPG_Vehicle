...
import numpy as np
from utils import plot_learning_curve #畫圖,用matplotlib畫標記軸
...
if __name__ == '__main__':
    ...
    #call MADDPG
    maddpg_agents = MADDPG(actor_dims, critic_dims, n_agents, n_actions, fc1=64, fc2=64, alpha=0.0001, beta=0.0001, gamma=0.95, scenario=scenario, chkpt_dir='tmp/maddpg/')
    ...
    for i in range(N_GAMES):
        ...
        while not any(done): #如果有任何done是false, not done=not false=true=1 ,while判斷為1就等於是無限迴圈,要終止的話就要所有done的值為true,not done才會為false
            ...
            obs_, reward, done, info = env.step(obs, actions, nextstep_count)
